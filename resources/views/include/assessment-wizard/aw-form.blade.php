<div class="row mt-5">
    <div class="col-md-8 pr-5 pl-4">
        <div class="kt-section__body">
            <!-- <h3 class="kt-section__title kt-section__title-lg">Customer Info:</h3> -->
            <div class="form-group row">
                <label class="col-3 col-form-label">Task Title</label>
                <div class="col-9">
                    <input class="form-control" type="text" value="Nick" style="background-image: url(&quot;data:image/png;base64,&quot;); background-repeat: no-repeat; background-attachment: scroll; background-size: 16px 18px; background-position: 98% 50%; cursor: auto;">
                </div>
            </div>
            <div class="form-group row">
                <label class="col-3 col-form-label">Process Title</label>
                <div class="col-9">
                    <input class="form-control" type="text" value="Watson">
                </div>
            </div>
            <div class="form-group row">
                <label class="col-3 col-form-label">Location</label>
                <div class="col-9">
                    <div class="input-group">
                        <div class="input-group-prepend"><span class="input-group-text"><i class="fa fa-map-signs"></i></span></div>
                        <select class="form-control ml-3">
                            <option>Sevron Ltd.</option>
                        </select>
                    </div>
                </div>
            </div>
            <div class="form-group row">
                <label class="col-md-3 col-form-label">Assessed on</label>
                <div class="col-md-9">
                    <div class="row">
                        <div class="col-md-5">
                            <div class="input-icon">
                                <i class="fa fa-calendar"></i>
                                <input type="date" class="form-control" placeholder="Left icon">
                            </div>
                        </div>
                        <div class="col-md-7">
                            <div class="form-group mb-0 row">
                                <label class="col-sm-2 col-lg-2 col-form-label">By</label>
                                <div class="col-sm-10 col-lg-10">
                                    <input type="text" value="Watson Towers" data-role="tagsinput" />   
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
            <div class="form-group row">
                <label class="col-md-3 col-form-label">Review date</label>
                <div class="col-md-9">
                    <div class="row">
                        <div class="col-md-5">
                            <div class="input-icon">
                                <i class="fa fa-calendar"></i>
                                <input type="date" class="form-control" placeholder="Left icon">
                            </div>
                        </div>
                        <div class="col-md-7">
                            <div class="form-group mb-0 row">
                                <label class="col-sm-4 col-lg-4 col-form-label">Reference no</label>
                                <div class="col-sm-8 col-lg-8">
                                    <input type="text" value="" class="form-control" />   
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>

            <div class="form-group row">
                <label class="col-md-3 col-form-label">Signed By</label>
                <div class="col-md-9">
                    <div class="row">
                        <div class="col-md-5">
                            <input type="text" value="John Doe" data-role="tagsinput" /> 
                        </div>
                        <div class="col-md-7">
                            <div class="form-group mb-0 row">
                                <label class="col-sm-4 col-lg-4 col-form-label">Revision no</label>
                                <div class="col-sm-8 col-lg-8">
                                    <input type="text" value="" class="form-control" />   
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
            <div class="form-group row">
                <label class="col-md-3 col-form-label">People Affected</label>
                <div class="col-md-9">
                    <input type="text" value="John Doe, Jane Doe" data-role="tagsinput" />                        
                </div>
            </div>
        </div>
    </div>
    <div class="col-md-4">
        <div class="risk-assessment__hazards">
            <h6>Hazards</h6>
            
            <div class="h_gal">

                <img src="{{URL::asset('assets/media/risk/hazzard1.png')}}"/>
                <img src="{{URL::asset('assets/media/risk/hazard2.png')}}"/>
                <img src="{{URL::asset('assets/media/risk/hazard3.png')}}"/>
                <img src="{{URL::asset('assets/media/risk/hazard4.png')}}"/>

                
            </div>
        </div>
        <div class="risk-assessment__hazards mt-5">
            <h6>PPE <i class="fa fa-exclamation-circle font-blue" aria-hidden="true" data-toggle="kt-tooltip" data-placement="right" title="" data-original-title="Sevron Information Here"></i></h6>

            
            <div class="h_gal">
                <img src="{{URL::asset('assets/media/risk/p1.png')}}" alt="">
                <img src="{{URL::asset('assets/media/risk/p1.png')}}" alt="">
            </div>
        </div>
    </div>
</div>